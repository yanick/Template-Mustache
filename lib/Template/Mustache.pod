...
of the L<https://github.com/mustache/spec|Mustache specs>.
...
=method new( template => $template, delimiters => [ $opening, $closing ], partials => \%partials )
...
C<template> is a string containing the Mustache template associated with the
object.

C<delimiters> is an optional arrayref holding the pair of delimiters used by
the template. It defaults to C<{{ }}>.

C<partials> is an optional hashref of partials to assign to the object. See
...
Returns the rendered template, given the optionally provided context.

The context can be a hashref. 
...
Add partial templates to the object. Partial values can be
strings holding Mustache templates, or coderefs. If a
partial is a coderef, it will be invoked with its name
as an argument and is expected to return a Mustache template.
